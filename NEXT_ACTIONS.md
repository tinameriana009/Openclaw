...
- shared `api` helpers now own more model resolution, auth resolution, bounded minimal web-evidence shaping, and shared recursive runtime construction
...
- fetch-state-aware provenance, approval-required query preservation, operator-facing web execution summaries, and degraded-path notes are now substantially more honest
...
- recent work added schemaVersion-aware artifact direction, chunk-neighbor retrieval, root-aware provenance, symbol-aware query heuristics, bounded semantic/language-intent signals, and intent-routing/document-continuity signals as practical retrieval lifts
