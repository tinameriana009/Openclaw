...
impl TraceLedger {
    ...
    pub fn to_json_value(&self) -> JsonValue {
        JsonValue::Object(BTreeMap::from([
            ("traceId".to_string(), JsonValue::String(self.trace_id.clone())),
            (
                "sessionId".to_string(),
...
impl TraceEvent {
    ...
    pub fn from_json_value(value: &JsonValue) -> Result<Self, TraceError> {
        ...
        Ok(Self {
            sequence: u32::try_from(expect_u64(object, "sequence")?).map_err(|_| {
                TraceError::Format("sequence is out of range for u32".to_string())
            })?,
...
fn optional_u64(object: &BTreeMap<String, JsonValue>, key: &str) -> Result<Option<u64>, TraceError> {
    match object.get(key) {
        Some(JsonValue::Null) | None => Ok(None),
        Some(JsonValue::Number(value)) => u64::try_from(*value)
            .map(Some)
            .map_err(|_| TraceError::Format(format!("numeric field {key} is out of range"))),
        Some(_) => Err(TraceError::Format(format!("field {key} must be a number or null"))),
    }
}
