...
- shared `api` helpers now own more model resolution, auth resolution, and bounded minimal web-evidence shaping
...
- fetch-state-aware provenance, approval-required query preservation, and degraded-path notes are now substantially more honest
...
- recent work added schemaVersion-aware artifact direction, chunk-neighbor retrieval, root-aware provenance, symbol-aware query heuristics, and bounded semantic/language-intent signals as practical retrieval lifts
