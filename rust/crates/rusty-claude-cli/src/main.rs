...
use api::{
    ...
    MessageResponse, NoopProviderRuntimeObserver, OutputContentBlock, PromptCache, ProviderClient,
...
fn parse_corpus_targeted_arg(input: &str) -> (Option<&str>, &str) {
    input
        .split_once("::")
        .map(|(corpus_id, remainder)| (Some(corpus_id.trim()), remainder.trim()))
        .unwrap_or((None, input.trim()))
}

fn resolve_corpus_for_command(
    cwd: &Path,
    requested_corpus_id: Option<&str>,
    command: &str,
) -> Result<String, Box<dyn std::error::Error>> {
    let corpora = list_corpora(cwd)?;
    if corpora.is_empty() {
        return Err("no corpora attached".into());
    }
    if let Some(corpus_id) = requested_corpus_id.filter(|value| !value.is_empty()) {
        if corpora.iter().any(|corpus| corpus.corpus_id == corpus_id) {
            return Ok(corpus_id.to_string());
        }
        return Err(format!("unknown corpus id {corpus_id}").into());
    }
    if corpora.len() == 1 {
        return Ok(corpora[0].corpus_id.clone());
    }
    let corpus_ids = corpora
        ...
        .join(", ");
    Err(format!(
        "multiple corpora attached; rerun as /corpus {command} <corpus-id> :: <value>\n  Corpus ids       {corpus_ids}"
    )
    .into())
}
...
fn run_corpus_command(
    ...
    if let Some(query) = args.strip_prefix("search ") {
        ...
        let corpus_id = resolve_corpus_for_command(&cwd, requested_corpus_id, "search")?;
        let result = search_corpus(&cwd, &corpus_id, query, 5, None)?;
    ...
    if let Some(chunk_id) = args.strip_prefix("slice ") {
        ...
        let corpus_id = resolve_corpus_for_command(&cwd, requested_corpus_id, "slice")?;
        let result = slice_corpus(&cwd, &corpus_id, Some(chunk_id), None, None)?;
        return Ok(serde_json::to_string_pretty(&result)?);
    }
...
fn run_trace_command(
    ...
    match (action, target, destination) {
        ...
        (Some("resume"), Some(target), None) => {
            resume_trace_for_operator(target, cwd, session_path)
        }
        _ => {
...
fn approve_trace_for_operator(
    ...
    let artifact = create_web_approval_artifacts(trace_target, cwd, session_path)?;
    Ok(format!(
        "Trace approval\n  Result           approval packet recorded\n  Trace            {}\n  Packet           {}\n  Review JSON      {}\n  Review Markdown  {}\n  Review HTML      {}\n  Review Status    {}\n  Review Log       {}\n  Review Index JSON {}\n  Review Index MD  {}\n  Review Index HTML {}\n  Pending queries  {}\n  Replay command   {}\n  Replay trace     /trace replay {}\n  Resume trace     /trace resume {}\n  Note             browser automation is still not available; rerun explicitly after approval or use /trace resume for a bounded approve-and-rerun loop",
        artifact.trace_path.display(),
        artifact.packet_path.display(),
        artifact.review_json_path.display(),
        artifact.review_markdown_path.display(),
        artifact.review_html_path.display(),
        artifact.review_status_path.display(),
        artifact.review_log_path.display(),
        artifact.review_index_json_path.display(),
        artifact.review_index_markdown_path.display(),
        artifact.review_index_html_path.display(),
        artifact.packet.pending_queries.join("; "),
        artifact.packet.replay_command,
        artifact.trace_path.display(),
        artifact.trace_path.display(),
    ))
}
...
#[derive(Debug)]
struct WebApprovalArtifacts {
    trace_path: PathBuf,
    packet_path: PathBuf,
    packet: WebApprovalPacket,
    review_json_path: PathBuf,
    review_markdown_path: PathBuf,
    review_html_path: PathBuf,
    review_status_path: PathBuf,
    review_log_path: PathBuf,
    review_index_json_path: PathBuf,
    review_index_markdown_path: PathBuf,
    review_index_html_path: PathBuf,
}
...
fn approval_packet_review_html_path(packet_path: &Path) -> PathBuf {
    packet_path.with_extension("review.html")
}

fn approval_packet_review_status_path(packet_path: &Path) -> PathBuf {
    packet_path.with_extension("review-status.json")
}

fn approval_packet_review_log_path(packet_path: &Path) -> PathBuf {
    packet_path.with_extension("review-log.md")
}

fn approval_review_index_json_path(cwd: &Path) -> PathBuf {
    cwd.join(".claw").join("web-approvals").join("index.json")
}

fn approval_review_index_markdown_path(cwd: &Path) -> PathBuf {
    cwd.join(".claw").join("web-approvals").join("index.md")
}
...
fn render_web_operator_review_html(
    packet: &WebApprovalPacket,
    packet_path: &Path,
    original_trace_path: &Path,
    replay_trace_path: Option<&Path>,
...
fn resolve_trace_target_path(trace_target: &str, cwd: &Path) -> PathBuf {
    let candidate = Path::new(trace_target);
    if candidate.is_absolute() {
        return candidate.to_path_buf();
    }
    let cwd_candidate = cwd.join(trace_target);
    if cwd_candidate.exists()
        || trace_target.contains(std::path::MAIN_SEPARATOR)
        || trace_target.ends_with(".json")
    {
        return cwd_candidate;
    }
    let trace_id = trace_target.trim();
    let approval_packet = cwd
        .join(".claw")
        .join("web-approvals")
        .join(format!("{trace_id}.json"));
    if approval_packet.exists() {
...
fn load_or_create_web_approval_packet(
    target: &str,
    cwd: &Path,
    session_path: Option<&Path>,
) -> Result<WebApprovalArtifacts, Box<dyn std::error::Error>> {
    let requested_path = resolve_trace_target_path(target, cwd);
    let is_packet_target = requested_path
        .file_name()
        .and_then(|value| value.to_str())
        .is_some_and(|name| name.ends_with(".json"))
        && requested_path.to_string_lossy().contains("web-approvals");
    if is_packet_target {
        ...
        let trace_path = requested_path
            .with_file_name(format!("{}.json", packet.trace_id))
            .parent()
            .and_then(|approvals_dir| approvals_dir.parent())
            .map(|claw_dir| {
                claw_dir
                    .join("trace")
                    .join(format!("{}.json", packet.trace_id))
            })
        ...
        let (review_index_json_path, review_index_markdown_path, review_index_html_path) =
            refresh_web_approval_review_index(cwd)?;
        return Ok(WebApprovalArtifacts {
            trace_path,
            packet_path,
            packet,
            review_json_path,
            ...
            review_status_path,
            review_log_path,
            review_index_json_path,
            review_index_markdown_path,
            review_index_html_path,
        });
    }
    create_web_approval_artifacts(target, cwd, session_path)
}

fn append_review_lifecycle_entry(
    packet: &WebApprovalPacket,
    packet_path: &Path,
    original_trace_path: &Path,
    replay_trace_path: Option<&Path>,
    ...
    let now_ms = std::time::SystemTime::now()
        ...
        .as_millis();
    let mut history = if review_status_path.exists() {
        let current: JsonValue = serde_json::from_str(&fs::read_to_string(&review_status_path)?)?;
        current["history"].as_array().cloned().unwrap_or_default()
    } else {
        Vec::new()
    };
    history.push(json!({
        ...
        "replayTrace": replay_trace_path.map(|path| path.display().to_string()),
        "pendingQueries": packet.pending_queries,
    }));
    let replay_count = history
        .iter()
        .filter(|entry| entry["replayTrace"].as_str().is_some())
        .count();
    let review_status = json!({
        "schemaVersion": 1,
        ...
        "approvalPacket": packet_path.display().to_string(),
        "originalTrace": original_trace_path.display().to_string(),
        "latestOperatorState": operator_state,
        "latestNextStep": next_step,
        "approvedAtMs": packet.approved_at_ms,
        "updatedAtMs": now_ms,
        "pendingQueries": packet.pending_queries,
        "replayCount": replay_count,
        "latestReplayTrace": replay_trace_path.map(|path| path.display().to_string()),
        "history": history,
    });
    fs::write(
        &review_status_path,
        serde_json::to_string_pretty(&review_status)?,
    )?;

    let mut lines = vec![
        "# Web approval lifecycle log".to_string(),
        "".to_string(),
        format!("- Trace ID: `{}`", packet.trace_id),
        format!("- Approval packet: `{}`", packet_path.display()),
        format!("- Original trace: `{}`", original_trace_path.display()),
        format!("- Latest operator state: {}", operator_state),
        format!("- Replay count: {}", replay_count),
        "".to_string(),
        "## History".to_string(),
        "".to_string(),
    ];
    for entry in review_status["history"].as_array().into_iter().flatten() {
        lines.push(format!(
            "- `{}` — {}",
            entry["recordedAtMs"].as_u64().unwrap_or(0),
            entry["operatorState"].as_str().unwrap_or("unknown")
        ));
        lines.push(format!(
            "  - next: {}",
            entry["nextStep"].as_str().unwrap_or("inspect review state")
        ));
        ...
        let pending = entry["pendingQueries"]
            ...
            .collect::<Vec<_>>();
        if !pending.is_empty() {
            lines.push(format!("  - pending queries: {}", pending.join(" | ")));
        }
    }
    lines.push("".to_string());
    lines.push(
        "Static lifecycle log only; browser automation and live web controls do not exist yet."
            .to_string(),
    );
    fs::write(&review_log_path, lines.join("\n"))?;
    Ok((review_status_path, review_log_path))
}

fn create_web_approval_artifacts(
    trace_target: &str,
    cwd: &Path,
    session_path: Option<&Path>,
) -> Result<WebApprovalArtifacts, Box<dyn std::error::Error>> {
    let trace_path = resolve_trace_target_path(trace_target, cwd);
    let trace = runtime::TraceLedger::read_from_path(&trace_path)?;
    ...
    if pending_queries.is_empty() {
        return Err(format!(
            "trace approval requires at least one approval-required web query: {}",
            trace_path.display()
        )
        .into());
    }
    ...
    let packet = WebApprovalPacket {
        ...
        replay_command,
        operator_note: Some("Approval has been recorded, but browser automation is still not available; rerun only collects bounded web evidence through the existing recursive path.".to_string()),
    };
    ...
    let review_json = json!({
        "schemaVersion": 1,
        "traceId": packet.trace_id.clone(),
        "sessionId": packet.session_id.clone(),
        ...
        "operatorState": operator_state,
        "nextStep": next_step,
        "pendingQueries": packet.pending_queries.clone(),
        "replayCommand": packet.replay_command.clone(),
        "operatorNote": packet.operator_note.clone(),
    });
    fs::write(
        &review_json_path,
    ...
    fs::write(
        &review_html_path,
        render_web_operator_review_html(
            &packet,
            &packet_path,
            &trace_path,
            None,
    ...
    let (review_status_path, review_log_path) = append_review_lifecycle_entry(
        &packet,
        &packet_path,
        &trace_path,
        None,
...
fn refresh_web_approval_review_index(
    ...
    let mut review_entries = entries
        ...
        .filter_map(|path| {
            ...
            let mut value: JsonValue = serde_json::from_str(&text).ok()?;
            let packet_path = value["approvalPacket"]
                .as_str()
                .map(PathBuf::from)
            ...
            if let Ok(status_text) = fs::read_to_string(&review_status_path) {
                if let Ok(status_value) = serde_json::from_str::<JsonValue>(&status_text) {
                    ...
                    value["updatedAtMs"] = status_value["updatedAtMs"].clone();
                    value["historyEntries"] = JsonValue::from(
    ...
    if review_entries.is_empty() {
        ...
        for entry in &review_entries {
            ...
            if let Some(review_status) = entry["reviewStatusPath"].as_str() {
                lines.push(format!("  - review status: {}", review_status));
            }
            if let Some(review_log) = entry["reviewLogPath"].as_str() {
                lines.push(format!("  - review log: {}", review_log));
            }
            if let Some(history_entries) = entry["historyEntries"].as_u64() {
                lines.push(format!("  - lifecycle entries: {}", history_entries));
            }
            if let Some(replay_count) = entry["replayCount"].as_u64() {
                lines.push(format!("  - replay count: {}", replay_count));
            }
    ...
    lines.push("".to_string());
    lines.push(
        "This is an on-disk operator review index, not a browser UI or automation surface."
            .to_string(),
    );
...
fn render_trace_approval_dashboard(cwd: &Path) -> Result<String, Box<dyn std::error::Error>> {
    let (index_json_path, index_markdown_path, index_html_path) =
        refresh_web_approval_review_index(cwd)?;
    let index_text = fs::read_to_string(&index_json_path)?;
    let index_value: JsonValue = serde_json::from_str(&index_text)?;
    let entries = index_value["entries"]
    ...
    let mut lines = vec![
        ...
        format!("  Review Index HTML  {}", index_html_path.display()),
        format!(
            "  Entries            {}",
            summary["entries"].as_u64().unwrap_or(entries.len() as u64)
        ),
        ...
        format!(
            "  Recorded replays   {}",
            summary["replayCount"].as_u64().unwrap_or(0)
        ),
    ];
    if entries.is_empty() {
        lines.push("  Note               no approval review artifacts exist yet".to_string());
...
fn replay_trace_for_operator(
    ...
    let requested_path = resolve_trace_target_path(trace_target, cwd);
    let (packet_path, packet) = load_web_approval_packet(trace_target, cwd)?;
    let original_trace_path = if requested_path.to_string_lossy().contains("web-approvals") {
        let candidate = cwd
            .join(".claw")
            .join("trace")
            .join(format!("{}.json", packet.trace_id));
    ...
    let _ = append_review_lifecycle_entry(
        &review_packet,
        &packet_path,
        &original_trace_path,
        replay_trace_path.as_deref(),
    ...
    Ok(format!(
        ...
        approval_packet_review_log_path(&packet_path).display(),
        review_index_json_path.display(),
        review_index_markdown_path.display(),
        review_index_html_path.display(),
...
fn resume_trace_for_operator(
    target: &str,
    cwd: &Path,
    session_path: Option<&Path>,
) -> Result<String, Box<dyn std::error::Error>> {
    let artifacts = load_or_create_web_approval_packet(target, cwd, session_path)?;
    let replay_output = replay_trace_for_operator(
        artifacts
            .packet_path
            .to_str()
            .ok_or("approval packet path must be utf8")?,
        cwd,
        session_path,
    )?;
    Ok(format!(
        "Trace resume\n  Result           approval recorded and rerun executed\n  Trace            {}\n  Packet           {}\n  Review JSON      {}\n  Review Markdown  {}\n  Review HTML      {}\n  Review Status    {}\n  Review Log       {}\n  Review Index JSON {}\n  Review Index MD  {}\n  Review Index HTML {}\n  Pending queries  {}\n  Note             browser automation is still not available; this only completes the bounded approve-and-rerun loop\n\n{}",
        artifacts.trace_path.display(),
        artifacts.packet_path.display(),
        artifacts.review_json_path.display(),
        artifacts.review_markdown_path.display(),
        artifacts.review_html_path.display(),
        artifacts.review_status_path.display(),
        artifacts.review_log_path.display(),
        artifacts.review_index_json_path.display(),
        artifacts.review_index_markdown_path.display(),
        artifacts.review_index_html_path.display(),
        artifacts.packet.pending_queries.join("; "),
        replay_output,
    ))
}
...
struct BuiltRuntime {
    runtime:
        Option<ConversationRuntime<ProviderRuntimeApiClient<CliRuntimeObserver>, CliToolExecutor>>,
    plugin_registry: PluginRegistry,
    plugins_active: bool,
}
...
impl Deref for BuiltRuntime {
    type Target =
        ConversationRuntime<ProviderRuntimeApiClient<CliRuntimeObserver>, CliToolExecutor>;
...
fn build_runtime_with_plugin_state(
    ...
    let mut runtime = build_provider_conversation_runtime_with_features_and_observer_for_session(
        session,
        model,
        tools,
        session_id,
        ...
        system_prompt,
        &feature_config,
        CliRuntimeObserver::new(emit_output, progress_reporter),
    )
    .map_err(std::io::Error::other)?;
    if emit_output {
...
mod tests {
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("/corpus [attach <path>|search"));
        assert!(help.contains("answer [<corpus-id> ::] <query>"));
        assert!(help.contains("slice [<corpus-id> ::] <chunk-id>"));
        assert!(help.contains("inspect <corpus-id>"));
    ...
    #[test]
    fn corpus_command_requires_explicit_selection_when_multiple_corpora_exist() {
        let _guard = env_lock();
        let root = temp_dir();
        let docs_a = root.join("docs-a");
        let docs_b = root.join("docs-b");
        fs::create_dir_all(&docs_a).expect("docs a dir");
        fs::create_dir_all(&docs_b).expect("docs b dir");
        fs::write(docs_a.join("guide.md"), "alpha corpus material\n").expect("write docs a");
        fs::write(docs_b.join("guide.md"), "beta corpus material\n").expect("write docs b");

        let first =
            runtime::attach_corpus(&root, &[docs_a], runtime::CorpusAttachOptions::default())
                .expect("attach first corpus");
        let second =
            runtime::attach_corpus(&root, &[docs_b], runtime::CorpusAttachOptions::default())
                .expect("attach second corpus");

        let error = with_current_dir(&root, || {
            run_corpus_command(
                Some("search corpus material"),
                ExecutionProfile::Balanced,
                Some("session-test"),
                None,
            )
            .expect_err("search should require an explicit corpus id")
            .to_string()
        });
        assert!(error.contains("multiple corpora attached"));
        assert!(error.contains(&first.corpus_id));
        assert!(error.contains(&second.corpus_id));

        let output = with_current_dir(&root, || {
            run_corpus_command(
                Some(&format!("search {} :: beta", second.corpus_id)),
                ExecutionProfile::Balanced,
                Some("session-test"),
                None,
            )
        ...
        assert!(output.contains("beta corpus material"));
