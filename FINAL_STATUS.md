...
The repo now has more honest bounded web semantics, fetch-state-aware provenance, approval-required query preservation, dedicated operator-facing web execution summaries, and better degraded-path reporting, but it still lacks a fully mature end-to-end web retrieval/execution path for recursive corpus-answer flows.

### 2. Child execution is not fully runtime-native
Provider-backed child execution has improved materially and more setup now lives in shared `api` abstractions, including more auth resolution, bounded web-adapter wiring, and shared recursive runtime construction, but some important seams still live in CLI-layer wiring instead of a cleaner shared runtime/provider abstraction.

### 3. Retrieval quality ceiling
Retrieval is no longer purely plain lexical: it now has stronger provenance, neighbor expansion, symbol-aware heuristics, bounded semantic/structure-aware query signals, and query-intent/document-continuity lifts. But for very large codebases or symbol-heavy workflows, the absence of deeper semantic/structure-aware retrieval will still become a bottleneck.
